...
float predict(const std::vector<float>& input_data, Ort::Session& session) {
    ...
    const char* input_names[] = {"dense_input"};  // Input name as obtained from the ONNX model
    const char* output_names[] = {"dense_3"};     // Output name as obtained from the ONNX model
...
int main() {
    ...
    load_scaler_params("scalers.txt", means, scales);
    ...
    Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "model_inference3");
    Ort::SessionOptions session_options;
    Ort::Session session(env, "ann_model.onnx", session_options); // Make sure the model file is in the same directory
